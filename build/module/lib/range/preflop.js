...
exports.any2pre = exports.PreflopRange = exports.isoPre2Combos = exports.handToIsoPre = exports.preflopStrengthOrder = void 0;
...
exports.preflopStrengthOrder = [...orderDesc].reverse();
// "isoPre" means AKo, 43s type representation
const isoPre = [];
for (let i = 12; i >= 0; i--) {
    for (let j = 12; j >= 0; j--) {
        const type = i === j ? '' : i > j ? 's' : 'o';
        isoPre.push(constants_1.RANKS[Math.max(i, j)] + constants_1.RANKS[Math.min(i, j)] + type);
    }
}
const isoPre2idx = Object.fromEntries(Object.keys(isoPre).map((i) => [isoPre[i], parseInt(i)]));
const handToIsoPre = (hand) => {
    const sorted = (0, sort_1.sortCards)([...hand]);
    const ranks = sorted.map((c) => (0, utils_1.getRank)(c));
    const suits = sorted.map((c) => (0, utils_1.getSuit)(c));
    const type = ranks[0] === ranks[1] ? '' : suits[0] === suits[1] ? 's' : 'o';
    return ranks.map((r) => constants_1.RANKS[r]).join('') + type;
};
exports.handToIsoPre = handToIsoPre;
exports.isoPre2Combos = {};
for (const hand of (0, utils_2.genCardCombinations)(2)) {
    const iso = (0, exports.handToIsoPre)(hand);
    exports.isoPre2Combos[iso] ??= [];
    exports.isoPre2Combos[iso].push(hand);
}
/** O(N) - Kadane's algorithm for https://en.wikipedia.org/wiki/Maximum_subarray_problem */
const maxSumSubarrayIdxs = (prices) => {
    let buy = 0;
    let bestProfit = 0;
...
class PreflopRange {
    ...
    static handCombos(hand) {
        return exports.isoPre2Combos[hand];
    }
    parseHand(hand) {
        if (typeof hand === 'string') {
            // if 27o convert to 72o
            if (constants_1.RANKS.indexOf(hand[0]) < constants_1.RANKS.indexOf(hand[1])) {
    ...
    static fromPercentiles(min, max) {
        const result = new PreflopRange();
        if (min > max)
            return result; // will be a empty range
        if (min < 0 || max < 0 || min > 1 || max > 1) {
            throw new Error(`min: ${min} and max: ${max} must be 0-1`);
        }
        const hands = exports.preflopStrengthOrder.slice(Math.floor(NUM_COMBOS * min), Math.floor(NUM_COMBOS * max));
    ...
    toPercentiles() {
        ...
        const avgComboWeight = this.weights.reduce((a, c) => a + c, 0) /
        ...
        return maxSumSubarrayIdxs(prices).map((idx) => idx / (this.weights.length - 1));
    }
    ...
    toString() {
        ...
        for (let i = 0; i < this.weights.length; i++) {
            if (this.weights[i]) {
                ...
                result.push(`${iso}${this.weights[i] === 1 ? '' : ':' + this.weights[i]}`);
            }
        }
        return result.join(',');
    }
}
exports.PreflopRange = PreflopRange;
exports.any2pre = PreflopRange.fromPercentiles(0, 1);
